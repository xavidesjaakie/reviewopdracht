...
<?php if (isset($_SESSION['user_id'])): ?>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // Laat de opties voor het bewerken of verwijderen van de review alleen zien voor de eigenaar of een admin
            if ($row['naam'] == $_SESSION['user_name'] || isset($_SESSION['is_admin'])) {
                echo "<a href='index.php?delete_review=" . $row["id"] . "'>Verwijder review</a> | ";
                echo "<a href='wijzig.php?review_id=" . $row["id"] . "'>Bewerk review</a>";
            }
            echo "<hr>";
        }
    } else {
